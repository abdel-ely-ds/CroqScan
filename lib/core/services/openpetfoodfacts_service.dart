...
/// Service for interacting with OpenPetFoodFacts API
/// 
/// Provides methods to:
/// - Fetch product details by barcode
/// - Search products by name, category, brand
/// - Parse API responses into [Product] models
/// 
/// API Documentation: https://openpetfoodfacts.org/data
class OpenPetFoodFactsService {
  ...
  static Future<Product?> fetchProductByBarcode(String barcode) async {
    try {
      debugPrint('🌐 Recherche sur OpenPetFoodFacts API: $barcode');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 1 && data['product'] != null) {
          debugPrint('✅ Produit trouvé sur l\'API!');
          return _parseProduct(data['product'], barcode);
        } else {
          debugPrint('❌ Produit non trouvé sur l\'API');
          return null;
        }
      } else {
  ...
  static Product _parseProduct(Map<String, dynamic> apiData, String barcode) {
    ...
    debugPrint('   📦 Produit parsé: $productName ($brands)');
    debugPrint('   🏷️  Catégories: $categories');
    debugPrint(
      '   🐾 Adapté pour: ${suitableFor.map((e) => e.displayName).join(", ")}',
    );
    debugPrint('   ⭐ Score calculé: $healthScore');
  ...
  static List<PetType> _detectPetTypes(String categories, List<String> tags) {
    ...
    debugPrint('   🏷️  Tags détectés: ${tags.take(3).join(", ")}${tags.length > 3 ? "..." : ""}');
    debugPrint('   🐾 Types détectés: ${types.map((e) => e.displayName).join(", ")}');
